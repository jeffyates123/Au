...
austerlitzModule.factory('landUnitsTurnSheetFactory', function () {
    return {
        attach: function ($scope, turnSheetFactory) {
            $scope.turnSheetPairIncludesBattalion = function (row, brigade, battalion, brigadeAField, battAField, brigadeBField, battBField) {
                    return !!(row && brigade && battalion)
                        && (($scope.sameNullableInt(row[brigadeAField], brigade.id) && $scope.sameNullableInt(row[battAField], battalion.slot))
                            || ($scope.sameNullableInt(row[brigadeBField], brigade.id) && $scope.sameNullableInt(row[battBField], battalion.slot)));
                };
            ...
            $scope.findMatchingExchangeRow = function (rows, leftBrigade, leftBattalion, rightBrigade, rightBattalion) {
                    return $scope.findMatchingPairRow(rows, leftBrigade.id, leftBattalion.slot, rightBrigade.id, rightBattalion.slot, 'brigadeA', 'battA', 'brigadeB', 'battB');
                };

            $scope.findMatchingMergeRow = function (rows, sourceBrigade, sourceBattalion, targetBrigade, targetBattalion) {
                    return $scope.findMatchingPairRow(rows, sourceBrigade.id, sourceBattalion.slot, targetBrigade.id, targetBattalion.slot, 'bridageA', 'battA', 'brigadeB', 'battB');
                };

            $scope.findMatchingPairRow = function (rows, brigadeA, battA, brigadeB, battB, brigadeAField, battAField, brigadeBField, battBField) {
                    for (var i = 0; rows && i < rows.length; i++) {
                        var row = rows[i];
                        var directMatch = $scope.sameNullableInt(row[brigadeAField], brigadeA)
                        ...
                        var reverseMatch = $scope.sameNullableInt(row[brigadeAField], brigadeB)
            ...
            $scope.findMatchingBrigadeOrFederationRow = function (rows, brigadeOrFederation) {
                    for (var i = 0; rows && i < rows.length; i++) {
                        if ($scope.sameNullableInt(rows[i].brigadeOrFederation, brigadeOrFederation)) {
                            return rows[i];
                        }
                    }
            ...
            $scope.findNextEmptyTurnSheetRowWithinLimit = function (rows, fields, maxRows) {
                    rows = rows || [];
