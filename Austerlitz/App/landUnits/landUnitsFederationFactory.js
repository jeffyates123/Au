    ...
    return {
        attach: function ($scope, turnSheetFactory) {
            $scope.persistFormFederationOrders = function (stagedOrders) {
                    turnSheetFactory.getTSFormFederations($scope.masterData.turnId).then(function (rows) {
                        ...
                        for (var i = 0; i < stagedOrders.length; i++) {
                            ...
                            var targetRow = $scope.findMatchingFormFederationRow(rows, order.itemNo)
                                || $scope.findNextEmptyTurnSheetRowWithinLimit(rows, ['itemNo', 'federation_Fleet'], 21);
            ...
            $scope.stageFormFederationOrder = function (order) {
                    if (!order || order.itemNo == null || !$scope.isValidTargetFederationNo(order.federation_Fleet)) {
                        return;
                    }
            ...
            $scope.getEffectiveFederationNo = function (brigade) {
                    ...
                    for (var i = stagedOrders.length - 1; i >= 0; i--) {
                        ...
                        if (order.type === 'federation' && $scope.sameNullableInt(order.sourceFederationNo, currentFederationNo)) {
                            return parseInt(order.federation_Fleet, 10) || 0;
                        }
                    }
            ...
            $scope.isValidTargetFederationNo = function (targetFederationNo) {
                    ...
                    if (targetFederationNo >= 61 && targetFederationNo <= 90 && $scope.isFederationNoValidForModalCoordinate(targetFederationNo)) {
                        $scope.formFederationModal.validationError = '';
                        return true;
                    }
                    ...
                    $scope.formFederationModal.validationError = 'Enter 0, an unused land federation number from 61 to 90, or a federation number that already exists only on this coordinate.';
                    return false;
                };
            ...
            $scope.openFormFederationModal = function (brigade, $event) {
                    ...
                    $scope.formFederationModal.validationError = '';
                    $scope.formFederationModal.coordinateBrigades = $scope.getSameCoordinateBrigades(brigade);
            ...
            $scope.saveFormFederationModal = function () {
                    if (!$scope.isValidTargetFederationNo($scope.getFormFederationTargetNo())) {
                        return;
                    }
