    ...
    return {
        attach: function ($scope, $q, turnSheetFactory) {
            $scope.replaySetUpAdditionalBrigades = function (rows, warnings) {
                    angular.forEach($scope.getFilledRowsInOrder(rows, ['brigadeNo', 'battType']), function (row) {
                        var brigade = $scope.getBrigadeById(row.brigadeNo);
                        var armyItem = $scope.getArmyItemByItemNo(row.battType);
                        if (!brigade) {
                            $scope.addReplayWarning(warnings, 'TS04', row, 'brigade not found: ' + row.brigadeNo);
                            return;
                        }
                        if (!armyItem) {
                            $scope.addReplayWarning(warnings, 'TS04', row, 'army item not found: ' + row.battType);
                            return;
                        }
            
                        var targetBattalion = $scope.findFirstFreeBattalion(brigade);
                        if (!targetBattalion) {
                            $scope.addReplayWarning(warnings, 'TS04', row, 'no free battalion slot for brigade ' + row.brigadeNo);
                            return;
                        }
            ...
            $scope.replayIncreaseHeadcount = function (rows, warnings) {
                    angular.forEach($scope.getFilledRowsInOrder(rows, ['brigadeOrFederation', 'increaseAmount']), function (row) {
                        var targetHeadcount = $scope.normalizeTargetHeadcount(row.increaseAmount);
                        var affectedBrigades = $scope.getBrigadeOrFederationAffectedBrigades(row.brigadeOrFederation);
                        if (!affectedBrigades.length) {
                            $scope.addReplayWarning(warnings, 'TS05', row, 'brigade/federation not found: ' + row.brigadeOrFederation);
                            return;
                        }
            
                        var scope = $scope.getReplayScope(row.brigadeOrFederation);
                        angular.forEach(affectedBrigades, function (brigade) {
                            $scope.applyHeadcountPlanToBrigade(brigade, targetHeadcount, scope, row.brigadeOrFederation);
                        });
                    });
                };

            $scope.replayIncreaseBrigadeXP = function (rows, warnings) {
                    angular.forEach($scope.getFilledRowsInOrder(rows, ['brigadeOrFederation']), function (row) {
                        var affectedBrigades = $scope.getBrigadeOrFederationAffectedBrigades(row.brigadeOrFederation);
                        if (!affectedBrigades.length) {
                            $scope.addReplayWarning(warnings, 'TS06', row, 'brigade/federation not found: ' + row.brigadeOrFederation);
                            return;
                        }
            
                        var scope = $scope.getReplayScope(row.brigadeOrFederation);
                        angular.forEach(affectedBrigades, function (brigade) {
                            $scope.applyTrainPlanToBrigade(brigade, scope, row.brigadeOrFederation);
                        });
                    });
                };
            ...
            $scope.replayExchangeBattalions = function (rows, warnings) {
                    angular.forEach($scope.getFilledRowsInOrder(rows, ['brigadeA', 'battA', 'brigadeB', 'battB']), function (row) {
                        ...
                        if (!left || !right) {
                            $scope.addReplayWarning(warnings, 'Exchange Battalions', row, 'brigade or battalion slot not found');
                            return;
                        }
                        if (!$scope.isSameCoordinate(left.brigade, right.brigade)) {
                            $scope.addReplayWarning(warnings, 'Exchange Battalions', row, 'brigades are not at the same coordinate');
                            return;
                        }
                        if (left.battalion.isNewAddition || right.battalion.isNewAddition) {
                            $scope.addReplayWarning(warnings, 'Exchange Battalions', row, 'newly added battalions cannot be exchanged in the same turn');
                            return;
                        }
                        if ($scope.isBattalionLockedForOrders(left.battalion) || $scope.isBattalionLockedForOrders(right.battalion)) {
                            $scope.addReplayWarning(warnings, 'Exchange Battalions', row, 'battalion already used by Exchange Battalions or Merge Battalions this turn');
                            return;
                        }
            
                        $scope.exchangeBattalions(left.brigade, left.battalion, right.brigade, right.battalion);
                    });
                };
            ...
            $scope.replayMergeBattalions = function (rows, warnings) {
                    angular.forEach($scope.getFilledRowsInOrder(rows, ['bridageA', 'battA', 'brigadeB', 'battB']), function (row) {
                        ...
                        if (!source || !target) {
                            $scope.addReplayWarning(warnings, 'Merge Battalions', row, 'brigade or battalion slot not found');
                            return;
                        }
                        if (!$scope.isSameCoordinate(source.brigade, target.brigade)) {
                            $scope.addReplayWarning(warnings, 'Merge Battalions', row, 'brigades are not at the same coordinate');
                            return;
                        }
                        if (source.battalion.isNewAddition || target.battalion.isNewAddition) {
                            $scope.addReplayWarning(warnings, 'Merge Battalions', row, 'newly added battalions cannot be merged in the same turn');
                            return;
                        }
                        if ($scope.isBattalionLockedForOrders(source.battalion) || $scope.isBattalionLockedForOrders(target.battalion)) {
                            $scope.addReplayWarning(warnings, 'Merge Battalions', row, 'battalion already used by Exchange Battalions or Merge Battalions this turn');
                            return;
                        }
                        if (!source.battalion.type || !target.battalion.type || source.battalion.type !== target.battalion.type) {
                            $scope.addReplayWarning(warnings, 'Merge Battalions', row, 'battalion types do not match');
                            return;
                        }
            
                        $scope.mergeBattalions(source.brigade, source.battalion, target.brigade, target.battalion);
                    });
                };
