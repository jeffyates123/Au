    ...
    attach: function ($scope) {
      $scope.calculateHeadcountResources = function (brigade, targetHeadcount) {
        ...
        angular.forEach(brigade.battalions, function (battalion) {
          var missingMen = $scope.getMissingHeadcount(
            battalion,
            targetHeadcount,
          );
          if (missingMen <= 0) {
          ...
          resources.ecPts += Math.round(
            Math.ceil(missingMen / 25) * ecPtsPer25,
          );
      ...
      $scope.applyHeadcountEfChanges = function (brigade, targetHeadcount) {
        angular.forEach(brigade.battalions, function (battalion) {
          ...
          var drop = $scope.getEfDrop(
            missingMen,
            $scope.getBattalionBaselineSize(battalion),
          );
      ...
      $scope.applyHeadcountPlanToBrigade = function (
        brigade,
        targetHeadcount,
        scope,
        sourceBrigadeId,
      ...
      $scope.applyTrainPlanToBrigade = function (
        brigade,
        scope,
        sourceBrigadeId,
      ) {
      ...
      $scope.recalculateBrigadeEffects = function (brigade) {
        ...
        if (brigade.headcountPlan) {
          $scope.addResources(
            resources,
            $scope.calculateHeadcountResources(
              brigade,
              brigade.headcountPlan.targetHeadcount,
            ),
          );
          $scope.applyHeadcountEfChanges(
            brigade,
            brigade.headcountPlan.targetHeadcount,
          );
        }
        ...
        if (brigade.trainPlan) {
          $scope.addResources(
            resources,
            $scope.calculateTrainResources(brigade),
          );
      ...
      $scope.updateBattalionDisplayState = function (battalion) {
        ...
        battalion.efDrop = 0;
        battalion.efIncrease = 0;
        battalion.isEfChanged =
          !isNaN(originalEf) && !isNaN(currentEf) && currentEf !== originalEf;
      ...
      $scope.getEffectiveTrainingHeadcount = function (brigade, battalion) {
        ...
        if (brigade.headcountPlan) {
          return Math.min(
            800,
            Math.max(currentSize, brigade.headcountPlan.targetHeadcount),
          );
        }
      ...
      $scope.calculateHeadcountPreview = function (
        affectedBrigades,
        targetHeadcount,
      ) {
        ...
        angular.forEach(affectedBrigades, function (brigade) {
          var resources = $scope.calculateHeadcountResources(
            brigade,
            targetHeadcount,
          );
      ...
      $scope.getBattalionDisplaySize = function (brigade, battalion) {
        ...
        if (brigade && brigade.headcountPlan) {
          return Math.min(
            800,
            Math.max(baseline, brigade.headcountPlan.targetHeadcount),
          );
        }
      ...
      $scope.calculateTrainResources = function (brigade) {
        ...
        angular.forEach(brigade.battalions, function (battalion) {
          ...
          $scope.addResources(
            resources,
            $scope.calculateBattalionTrainingResources(brigade, battalion),
          );
        });
      ...
      $scope.calculateBattalionTrainingResources = function (
        brigade,
        battalion,
      ) {
        ...
        var headcount = $scope.getEffectiveTrainingHeadcount(
          brigade,
          battalion,
        );
      ...
      $scope.calculateTrainPreview = function (affectedBrigades) {
        ...
        angular.forEach(affectedBrigades, function (brigade) {
          angular.forEach(brigade.battalions, function (battalion) {
            if ($scope.canTrainBattalion(battalion)) {
              var resources = $scope.calculateBattalionTrainingResources(
                brigade,
                battalion,
              );
              preview.ld += parseInt(resources.ld, 10) || 0;
      ...
      $scope.isBrigadeAtBarracks = function (brigade) {
        ...
        for (var i = 0; i < barracks.length; i++) {
          if (
            parseInt(barracks[i].x, 10) === x &&
            parseInt(barracks[i].y, 10) === y
          ) {
            return true;
          }
        }
