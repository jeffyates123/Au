...
austerlitzModule.factory(
  ...
  function ($q, turnSheetValueRulesFactory, ts01TransferGoodsUtilsFactory) {
    var LAND_UNITS_MANAGED_TS01_KEY_PREFIX =
      ...
      attach: function ($scope, turnSheetFactory) {
        $scope.refreshSetUpCostRowsFromTurnSheets = function () {
          ...
          return $q
            .all([
              turnSheetFactory
                .getTSIncreaseHeadcount(turnId)
                .then(function (rows) {
                  $scope.tsIncreaseHeadcountList = rows || [];
                }),
              turnSheetFactory
                .getTSIncreaseBrigadeXP(turnId)
                .then(function (rows) {
            ...
            .then(function () {
              if (
                typeof $scope.recalculateTransferGoodsForSetUpBrigades ===
                "function"
              ) {
        ...
        $scope.getWarehouseNoForCoordinate = function (x, y) {
          return ts01TransferGoodsUtilsFactory.getWarehouseNoForCoordinate(
            x,
            y,
          );
        };
        ...
        $scope.syncTransferGoodsForLandUnitsPlans = function () {
          ...
          angular.forEach($scope.brigadeRows, function (brigade) {
            ...
            var warehouse = $scope.getWarehouseNoForCoordinate(
              brigade && brigade.source ? brigade.source.x_OrState : null,
              brigade && brigade.source ? brigade.source.y_OrFleet : null,
            );
            if (!depot || !warehouse) {
              return;
            }
            ...
            if (brigade.headcountPlan) {
              var headcountCost = $scope.calculateHeadcountResources(
                brigade,
                brigade.headcountPlan.targetHeadcount,
              );
          ...
          return turnSheetFactory
            .getTSTransferGoods($scope.masterData.turnId)
            .then(function (rows) {
              rows = rows || [];
              var ts01MaxRows = LAND_UNITS_TS01_MAX_ROWS;
              if (transferLines.length > ts01MaxRows) {
                alert(
                  "Cannot apply this Headcount/Train change because it would require " +
                    transferLines.length +
                    " TS01 rows, but TS01 supports a maximum of " +
                    ts01MaxRows +
                    ".",
                );
                return null;
              ...
              var managedRows = managedOrderNos
              ...
              var freeRows = rows
                .filter(function (row) {
                  ...
                  return (
                    managedOrderNos.indexOf(orderNo) < 0 &&
                    $scope.isTransferGoodsRowEmpty(row)
                  );
                })
                .sort(function (left, right) {
                  return toInt(left.orderNo, 0) - toInt(right.orderNo, 0);
              ...
              for (var i = 0; i < transferLines.length; i++) {
                if (i < managedRows.length) {
                  ...
                  var created = $scope.findNextEmptyTurnSheetRowWithinLimit(
                    rows,
                    [
                      "from",
                      "to",
              ...
              if (targetRows.length < transferLines.length) {
                alert(
                  "Cannot apply this Headcount/Train change because TS01 has no free row within the first " +
                    ts01MaxRows +
                    " rows.",
                );
                return null;
              ...
              for (
                var staleIdx = targetRows.length;
                staleIdx < managedRows.length;
                staleIdx++
              ) {
              ...
              var nextManaged = targetRows
                .map(function (row) {
                  return toInt(row.orderNo, 0);
                })
        ...
        $scope.persistHeadcountOrder = function (
          brigade,
          scope,
          targetHeadcount,
        ) {
          ...
          return turnSheetFactory
            .getTSIncreaseHeadcount($scope.masterData.turnId)
            .then(function (rows) {
              rows = rows || [];
              var targetRow =
                $scope.findMatchingBrigadeOrFederationRow(
                  rows,
                  brigadeOrFederation,
                ) ||
                $scope.findNextEmptyTurnSheetRowWithinLimit(
                  rows,
                  ["brigadeOrFederation", "increaseAmount"],
                  12,
                );
              ...
              return turnSheetFactory
                .postTSRecords(rows, "IncreaseHeadcount")
        ...
        $scope.openHeadcountModal = function (brigade) {
          ...
          if (!$scope.isBrigadeAtBarracks(brigade)) {
            alert(
              "Headcount can only be increased when the brigade is at one of your barracks.",
            );
            return;
          }
          ...
          $scope.headcountModal.targetHeadcount = brigade.headcountPlan
            ? brigade.headcountPlan.targetHeadcount
            : 800;
          $scope.headcountModal.scope = brigade.headcountPlan
            ? brigade.headcountPlan.scope
            : "brigade";

          if (
            $scope.headcountModal.scope === "federation" &&
        ...
        $scope.closeHeadcountModal = function () {
          ...
          $scope.headcountModal.preview =
            $scope.calculateEmptyHeadcountPreview();
        };

        $scope.onHeadcountScopeChanged = function () {
          if (
            $scope.headcountModal.scope === "federation" &&
            !$scope.canApplyFederationScope($scope.headcountModal.brigade)
          ) {
            $scope.headcountModal.scope = "brigade";
          }
          $scope.refreshHeadcountPreview();
        ...
        $scope.refreshHeadcountPreview = function () {
          ...
          var targetHeadcount = $scope.normalizeTargetHeadcount(
            $scope.headcountModal.targetHeadcount,
          );
          var affectedBrigades = $scope.getHeadcountAffectedBrigades(
            brigade,
            $scope.headcountModal.scope,
          );
          $scope.headcountModal.preview = $scope.calculateHeadcountPreview(
            affectedBrigades,
            targetHeadcount,
          );
        };
        ...
        $scope.applyHeadcountPlan = function () {
          ...
          if (
            $scope.hasAnyLockedBrigade(affectedBrigades, "Increase headcount")
          ) {
            return;
          }
        ...
        $scope.clearHeadcountPlan = function () {
          ...
          angular.forEach(
            affectedBrigades,
            $scope.clearHeadcountPlanFromBrigade,
          );
        ...
        $scope.openTrainModal = function (brigade) {
          ...
          if (!$scope.isBrigadeAtBarracks(brigade)) {
            alert(
              "Training can only be done when the brigade is at one of your barracks.",
            );
            return;
          }
          ...
          $scope.trainModal.scope = brigade.trainPlan
            ? brigade.trainPlan.scope
            : "brigade";
        ...
        $scope.onTrainScopeChanged = function () {
          if (
            $scope.trainModal.scope === "federation" &&
            !$scope.canApplyFederationScope($scope.trainModal.brigade)
          ) {
            $scope.trainModal.scope = "brigade";
          }
          $scope.refreshTrainPreview();
        ...
        $scope.refreshTrainPreview = function () {
          ...
          var affectedBrigades = $scope.getHeadcountAffectedBrigades(
            brigade,
            $scope.trainModal.scope,
          );
          $scope.trainModal.preview =
            $scope.calculateTrainPreview(affectedBrigades);
        };
